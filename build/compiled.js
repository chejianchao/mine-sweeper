  ...
  render() {
    return DiffDom.createElement(
      ...
      DiffDom.createElement(
        "button",
        { id: "start-btn", type: "button", "class": "action-btn js-new-game", "aria-label": "New game", onClick: this.restart.bind(this) },
        DiffDom.createElement(
          "span",
          { "class": "default-emoji" },
          this.game.stateEmoji()
        ),
        DiffDom.createElement("span", { id: "result", "class": "result-emoji" })
      ),
      DiffDom.createElement(
        "div",
        null,
...
class Minesweeper extends Component {
  ...
  render() {
    ...
    const dom = DiffDom.createElement(
      'div',
      { className: 'grid', style: `grid-template-columns: repeat(${this.m}, auto)`, oncontextmenu: this.onContextmenu.bind(this) },
      this.panel.map((row, ri) => {
        //console.log(row.map(n=>CELL_ICON[n]).join(" "))
        return row.map((n, ci) => DiffDom.createElement(
          'div',
          { className: 'cell', 'data-id': ri * 1000 + ci, onClick: this.onSearch.bind(this) },
          CELL_ICON[n]
        ));
      })
    );
  ...
  search(x, y) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (this.checkMine(nx, ny)) mine_pos.push([nx, ny]);else empty_slot.push([nx, ny]);
    }
  ...
  checkWin() {
    ...
    let flag_num = 0,
        bomb_num = Object.keys(this.mine_pos).length,
        flag_bomb_num = 0;
    for (let i = 0; i < this.n; i++) {
      for (let j = 0; j < this.m; j++) {
        if (this.panel[i][j] == FLAG_ICON) {
          if (this.matrix[i][j] == 1) flag_bomb_num++;
          flag_num++;
        }
        if (this.panel[i][j] == 0) {
  ...
  setFlag(x, y) {
    ...
    if (val == FLAG_ICON) this.panel[x][y] = 0;else if (val == 0) this.panel[x][y] = FLAG_ICON;
  }
  reset(n, m) {
    this.n = n || 10;
    this.m = m || 10;
    ...
    for (let i = 0; i < this.n; i++) {
      ...
      for (let j = 0; j < this.m; j++) {
        ...
        if (Math.random() > mine_rate || Math.abs(i - x) <= 1 && Math.abs(j - y) <= 1) row.push(0);else {
  ...
  stateEmoji() {
    if (this.state === GAME_STATE_WIN) return '😎';else if (this.state === GAME_STATE_GAMEOVER) return '😵';else return '😀';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //start()
  /*
  const game = new Minesweeper(10, 20);
  ...
  const start = document.getElementById("start-btn");
   panel.addEventListener("click", (e)=>{
    ...
    const y = id%1000;
    game.search(x,y);
    //panel.innerHTML = game.render()
    DiffDom.renderDom(game, panel)
    document.querySelector('.default-emoji').innerHTML = game.stateEmoji()
  })
